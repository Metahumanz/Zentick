...
const App: React.FC = () => {
  ...
  const [language, setLanguage] = useState<Language>(() => {
    ...
    if (browserLang === 'zh') return Language.ZH;
    if (browserLang === 'de') return Language.DE;
    return Language.EN;
  });
  ...
  useEffect(() => {
    ...
    if (isSettingsOpen || isLangMenuOpen) {
      document.addEventListener('mousedown', handleClickOutside);
    }
    return () => document.removeEventListener('mousedown', handleClickOutside);
  }, [isSettingsOpen, isLangMenuOpen]);
  ...
  const handleDoubleTap = (e: React.MouseEvent) => {
    ...
    if (target.closest('button') || target.closest('input') || target.closest('select') || target.closest('a') || target.closest('.settings-modal') || target.closest('.lang-menu')) {
      return;
    }
    toggleFullscreen();
      ...
      <div className="relative z-10 flex flex-col h-full pointer-events-none">
        ...
        >
          ...
          <div className="flex gap-4 items-center relative">
            ...
            {/* Language Toggle (Earth Icon) */}
            <div className="relative">
              <button
                ref={langBtnRef}
                onClick={() => setIsLangMenuOpen(!isLangMenuOpen)}
                className={`w-10 h-10 rounded-full bg-white/10 dark:bg-black/20 backdrop-blur-md flex items-center justify-center border border-white/20 hover:bg-white/20 transition-all cursor-pointer ${isLangMenuOpen ? 'bg-white text-slate-900' : ''}`}
              ...
              {isLangMenuOpen && (
                <div
                  ref={langMenuRef}
                  className="absolute top-12 right-0 w-32 bg-white/90 dark:bg-slate-800/90 backdrop-blur-xl border border-white/20 rounded-xl p-2 shadow-xl z-50 lang-menu animate-scale-up flex flex-col gap-1"
                >
                  {[
                    { code: Language.EN, label: 'English' },
                    { code: Language.ZH, label: '中文' },
                    { code: Language.DE, label: 'Deutsch' },
                  ].map((lang) => (
                    <button
                      key={lang.code}
                      onClick={() => toggleLanguage(lang.code)}
                      className={`px-3 py-2 text-sm font-bold rounded-lg text-left transition-colors cursor-pointer flex items-center justify-between
                                    ${language === lang.code
